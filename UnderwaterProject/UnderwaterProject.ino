...
MPU6050_PID_Controller pidController(9, 10, 11, 12, 13, 14); // ESC pins connected to these pins

void setup() {
  setupSensorsAndActuators();
  pidController.setup(); // Initialize PID controller
}
...
void loop() {
  ...
  if (currentMillis - loopPreviousMillis >= loopInterval) {
    ...
    // Update PID controller (for ESC control)
